...
# 4. [수정됨] 재시도 함수 (변수 에러 해결!)
def generate_with_retry(model_name, prompt):
    max_retries = 3
    last_error = None # 에러를 기억할 변수 미리 생성
    ...
    for attempt in range(max_retries):
        try:
            ...
            last_error = e # 에러 내용 저장
            time.sleep(2) 
            continue 
            
    # 3번 다 실패하면 저장해둔 에러 메시지 출력
...
if prompt := st.chat_input("증상을 입력하세요"):
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            if "429" in str(e):
                st.error("⚠️ [중요] 'API 키 한도 초과'입니다.")
                st.info("👉 해결책: 아까 말씀드린 대로 '새 구글 계정'이나 '새 프로젝트'에서 키를 다시 발급받아 Secrets에 넣으시면 바로 해결됩니다.")
